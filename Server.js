...
# one-file scaffold + git push + zip for GitHub copy-paste
# Save this file as build_and_publish.sh, make executable, run:
# chmod +x build_and_publish.sh
# ./build_and_publish.sh <repo-name> <visibility> [github-org]
#
# Example:
# ./build_and_publish.sh b2b-web3-rpc-service private
...
command -v gh >/dev/null 2>&1 || echo "gh not found; repo creation will be skipped"
...
# deploy manifests (already created above in deploy/)
# create initial README
cat > README.md <<'README'
# b2b-web3-rpc-service

Backend: Node.js + TypeScript + Express + Prisma
Frontend: React + Vite + Tailwind (admin)
Docker Compose + Kubernetes manifests + GitHub Actions CI

Seed admin: admin@example.com / Admin123!
README

# git commit & create repo
git add .
git commit -m "Initial scaffold for b2b-web3-rpc-service" || true
...
if command -v gh >/dev/null 2>&1; then
  ...
  echo "gh CLI not found; repository creation skipped. Add remote and push manually."
fi

# create zip
ZIP_NAME="${REPO_NAME}.zip"
git archive -o "$ZIP_NAME" HEAD 2>/dev/null || zip -r "$ZIP_NAME" . -x .git\* node_modules\* dist\* "*.env" "*.env.local" .DS_Store || true
